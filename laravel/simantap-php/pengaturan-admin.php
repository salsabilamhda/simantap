...
<div class="space-y-6 max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onclick="document.getElementById('add-admin-modal').classList.remove('hidden')" class="btn-gold-primary px-4 py-2.5 text-xs uppercase tracking-wider gap-2">
    ...
    <div class="bg-white rounded-3xl p-6 border border-teal-100 shadow-card-custom">
        <?php if (empty($admins)): ?>
            <div class="py-8 text-center text-gray-400 text-xs font-medium">Belum ada akun admin terdaftar.</div>
        <?php else: ?>
        <div class="divide-y divide-gray-100">
            ...
            <div class="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-extrabold text-gray-900 text-sm"><?= h($admin['name']) ?></div>
...
<div id="add-admin-modal" class="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 hidden">
    <div class="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl space-y-4">
        ...
        <form method="POST" action="<?= BASE_URL ?>/actions/admin-store.php" class="space-y-3">
            ...
            <button type="submit" class="btn-gold-primary py-2.5 text-xs uppercase tracking-wider w-full mt-4 justify-center">Buat Akun Admin</button>
        </form>
    </div>
</div>
